...
def resize_images_in_docx(doc_path, output_path, log_func):
    try:
            ...
            for run in paragraph.runs:
                if run._element.xpath('.//pic:pic'):
                    for drawing in run._element.xpath('.//w:drawing'):
                        ...
                        new_width_emu = int(5 * 914400)  # 1 inch = 914400 EMUs
        ...
        if resized_any:
            ...
            log_func(f"SUCCESS: {os.path.basename(doc_path)} -> {os.path.basename(output_path)}")
        else:
            log_func(f"NO IMAGES FOUND: {os.path.basename(doc_path)}")
